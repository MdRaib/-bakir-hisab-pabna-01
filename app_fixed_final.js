...
async function driveFetch(url,opts={},retryAuth=true){
  let r=await fetch(url,{...opts,headers:{...(opts.headers||{}),Authorization:'Bearer '+accessToken}});
  if(r.status===401){
    accessToken=null;
    if(retryAuth&&await ensureDriveAccess()) return driveFetch(url,opts,false);
    throw Error('AUTH');
  }
...
async function ensureDriveFolder(){
  ...
  let q=encodeURIComponent("name='সহজ হিসাব' and mimeType='application/vnd.google-apps.folder' and trashed=false and 'root' in parents");
  let f=await driveFetch(`https://www.googleapis.com/drive/v3/files?q=${q}&spaces=drive&fields=files(id,name)`);
  if(f.files?.length)return driveFolderId=f.files[0].id;
  let x=await driveFetch('https://www.googleapis.com/drive/v3/files',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:'সহজ হিসাব',mimeType:'application/vnd.google-apps.folder',parents:['root']})});
  return driveFolderId=x.id;
}
async function uploadOrUpdateDrive(name,mime,blob,folder){
  let q=encodeURIComponent(`name='${name}' and '${folder}' in parents and trashed=false`);
  let f=await driveFetch(`https://www.googleapis.com/drive/v3/files?q=${q}&spaces=drive&fields=files(id,name)`);
  let old=f.files?.[0];
  if(old){
    await driveFetch(`https://www.googleapis.com/upload/drive/v3/files/${old.id}?uploadType=media`,{method:'PATCH',headers:{'Content-Type':mime},body:blob});
    return old.id;
  }
  let form=new FormData();
  form.append('metadata',new Blob([JSON.stringify({name,mimeType:mime,parents:[folder]})],{type:'application/json'}));
  form.append('file',blob);
  let r=await fetch('https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,name',{method:'POST',headers:{Authorization:'Bearer '+accessToken},body:form});
  if(r.status===401){
    accessToken=null;
    if(await ensureDriveAccess()) return uploadOrUpdateDrive(name,mime,blob,folder);
    throw Error('AUTH');
  }
  if(!r.ok)throw Error(await r.text());
...
async function driveBackup(silent=false){
  ...
  driveBusy=true;
  const backupSnapshot=JSON.stringify(db);
  try{
    ...
    // If data changed while this backup was running, keep dirty=true so it is backed up next.
    if(JSON.stringify(db)===backupSnapshot){
      backupDirty=false;localStorage.setItem('shudhu-baki-backup-dirty','0');
    }else{
      backupDirty=true;localStorage.setItem('shudhu-baki-backup-dirty','1');
    }
  ...
  finally{driveBusy=false;scheduleAutoBackup()}
}
