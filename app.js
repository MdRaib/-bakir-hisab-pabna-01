...
function configureSupportLinks() {
  ...
  const bkash = $('#lockBkash');
  const nagad = $('#lockNagad');
  if (bkash) bkash.textContent = cfg.BKASH_NUMBER || '01XXXXXXXXX';
  if (nagad) nagad.textContent = cfg.NAGAD_NUMBER || '01XXXXXXXXX';
...
async function checkSubscriptionStatus() {
  ...
  const url = String(typeof ADMIN_API_URL !== 'undefined' ? ADMIN_API_URL : '').trim();
  const siteId = String(typeof SITE_ID !== 'undefined' ? SITE_ID : '').trim();
