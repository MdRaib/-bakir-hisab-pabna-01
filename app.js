...
// Resolve SITE_ID for both normal browser mode and installed PWA mode.
// In Chrome the client link contains ?site_id=..., but an installed PWA
// may reopen without the query string. Persist the last valid SITE_ID so
// the installed app continues using the same client instance.
const SITE_ID_STORAGE_KEY = 'shudhu-baki-site-id';
const SITE_ID_COOKIE_KEY = 'shudhu_baki_site_id';

function readSiteIdCookie(){
  try{
    const m=document.cookie.match(new RegExp('(?:^|; )'+SITE_ID_COOKIE_KEY+'=([^;]*)'));
    return m ? decodeURIComponent(m[1]).trim() : '';
  }catch(e){return ''}
}

function persistSiteId(id){
  id=String(id||'').trim();
  if(!id)return;
  try{localStorage.setItem(SITE_ID_STORAGE_KEY,id)}catch(e){}
  try{sessionStorage.setItem(SITE_ID_STORAGE_KEY,id)}catch(e){}
  try{
    document.cookie=SITE_ID_COOKIE_KEY+'='+encodeURIComponent(id)+'; path=/; max-age=31536000; SameSite=Lax';
  }catch(e){}
}

function getStoredSiteId(){
  try{
    const local=String(localStorage.getItem(SITE_ID_STORAGE_KEY)||'').trim();
    if(local)return local;
  }catch(e){}
  try{
    const session=String(sessionStorage.getItem(SITE_ID_STORAGE_KEY)||'').trim();
    if(session)return session;
  }catch(e){}
  return readSiteIdCookie();
}

function getSiteIdFromAnyUrl(){
  try{
    const params=new URLSearchParams(window.location.search);
    const direct=String(
      params.get('site_id')||params.get('siteId')||params.get('site')||''
    ).trim();
  ...
  try{
    ...
    const h=String(
      hash.get('site_id')||hash.get('siteId')||hash.get('site')||''
    ).trim();
    if(h)return h;
  }catch(e){}
...
/*
 * SITE_ID priority:
 * 1) Current URL (new client link wins)
 * 2) Previously persisted PWA/client state
 * 3) Static APP_CONFIG fallback
 *
 * SITE_ID is an identifier, not a secret. localStorage is used for
 * persistence across normal browser launches and installed-PWA launches.
 */
function resolveSiteId(){
  let id=getSiteIdFromAnyUrl();
  if(!id)id=getStoredSiteId();

  const configSiteId=String(
    (typeof APP_CONFIG!=='undefined' && APP_CONFIG && APP_CONFIG.SITE_ID)||''
  ).trim();

  if(!id)id=configSiteId;
  if(id)persistSiteId(id);
...
function showMissingSiteIdSetup(){
  setTimeout(()=>{
    document.body.innerHTML=`
      <div style="position:fixed;inset:0;display:flex;align-items:center;justify-content:center;background:#f6f7fb;z-index:999999;font-family:Arial,sans-serif;padding:20px;box-sizing:border-box;">
        <div style="width:min(420px,100%);background:#fff;border-radius:18px;padding:28px;box-shadow:0 10px 35px rgba(0,0,0,.10);text-align:center;">
          <h2 style="margin:0 0 10px">অ্যাপ সেটআপ</h2>
          <p style="color:#555;line-height:1.6;margin:0 0 18px">
            এই ইনস্টল করা অ্যাপের জন্য SITE_ID পাওয়া যায়নি।
            আপনার ক্লায়েন্ট লিংকের <b>site_id</b> অংশটি নিচে লিখুন।
          </p>
          <input id="pwaSiteIdInput" type="text" autocomplete="off"
            placeholder="যেমন: client_pabna_01"
            style="width:100%;box-sizing:border-box;padding:13px 14px;border:1px solid #ddd;border-radius:10px;font-size:16px;margin-bottom:12px">
          <button id="pwaSiteIdSave"
            style="width:100%;padding:13px;border:0;border-radius:10px;background:#111;color:#fff;font-size:16px;font-weight:700;cursor:pointer">
            অ্যাপ চালু করুন
          </button>
          <p id="pwaSiteIdError" style="display:none;color:#c62828;margin:12px 0 0">
            SITE_ID লিখুন।
          </p>
        </div>
      </div>`;

    const input=document.getElementById('pwaSiteIdInput');
    const btn=document.getElementById('pwaSiteIdSave');

    const submit=()=>{
      const id=String(input?.value||'').trim();
      const error=document.getElementById('pwaSiteIdError');
...
if(!SITE_ID)showMissingSiteIdSetup();
 ...
 *
 * Important:
 * - The server is authoritative whenever a real network request succeeds.
 * - A valid cached license is only a fallback for network/offline failures.
 * - The cache is tied to SITE_ID, so one client's cached license is never
 *   reused for another client's SITE_ID.
 * - CACHE_TTL controls how long a successful server verification can be
 *   trusted while the device cannot reach ADMIN_API_URL.
 */
const SUBSCRIPTION_CACHE_PREFIX='shudhu-baki-subscription-v1:';
const SUBSCRIPTION_CACHE_TTL_MS=30*24*60*60*1000;

function getSubscriptionCacheKey(siteId=SITE_ID){
  return SUBSCRIPTION_CACHE_PREFIX+String(siteId||'').trim();
}

function readSubscriptionCache(siteId=SITE_ID){
  if(!siteId)return null;

  try{
    const raw=localStorage.getItem(getSubscriptionCacheKey(siteId));
    if(!raw)return null;

    const cached=JSON.parse(raw);
    if(!cached || cached.site_id!==String(siteId).trim())return null;

    const verifiedAt=Number(cached.verified_at||0);
    const cacheExpiresAt=Number(cached.cache_expires_at||0);
    const expiry=String(cached.expiry_date||'').slice(0,10);
    const status=String(cached.status||'').trim().toLowerCase();

    if(status!=='active' || !isExpiryValid(expiry))return null;
    if(!verifiedAt || !cacheExpiresAt || Date.now()>cacheExpiresAt)return null;
    ...
    console.warn('Subscription cache read failed:',e);
    return null;
  }
}

function writeSubscriptionCache(siteId,result){
  if(!siteId || !result)return;

  const expiry=String(result.expiry_date||'').trim().slice(0,10);
  const status=String(result.status||'').trim().toLowerCase();

  // Never cache an invalid/expired server response as an offline pass.
  if(status!=='active' || !isExpiryValid(expiry))return;
  ...
  const cache={
    ...
    expiry_date:expiry,
    verified_at:now,
    cache_expires_at:now+SUBSCRIPTION_CACHE_TTL_MS
  };

  try{
    localStorage.setItem(getSubscriptionCacheKey(siteId),JSON.stringify(cache));
  }catch(e){
    console.warn('Subscription cache write failed:',e);
  }
}

function clearSubscriptionCache(siteId=SITE_ID){
  if(!siteId)return;
  try{localStorage.removeItem(getSubscriptionCacheKey(siteId))}catch(e){}
}

function applyActiveSubscription(expiry,state='Active'){
  subscriptionState=state;
  subscriptionExpiry=String(expiry||'').slice(0,10);
  subscriptionValid=true;
  setSubscriptionLock(false);
  return true;
}

function applyLockedSubscription(state,reason){
  subscriptionState=state;
  subscriptionExpiry='';
  subscriptionValid=false;
  setSubscriptionLock(true,reason);
  return false;
}
...
async function checkSubscriptionStatus(){
  ...
  // Keep the UI locked only while this function determines the state.
  setSubscriptionLock(true,'Subscription status যাচাই করা হচ্ছে…');

  const url=String(
    (typeof ADMIN_API_URL!=='undefined' && ADMIN_API_URL)||''
  ).trim();
  const siteId=String(SITE_ID||'').trim();

  /*
   * Missing configuration is a real configuration error, not an offline
   * condition. Do not silently grant access in this case.
   */
  if(!url || url.includes('PASTE_') || !siteId){
    return applyLockedSubscription(
      'Unavailable',
      'Subscription API অথবা SITE_ID পাওয়া যায়নি।'
    );
  }

  /*
   * If the browser explicitly reports offline, avoid a guaranteed failed
   * fetch and immediately try the per-SITE_ID cache.
   */
  if(!navigator.onLine){
    const cached=readSubscriptionCache(siteId);

    if(cached){
      return applyActiveSubscription(cached.expiry_date,'Offline Cache');
    }

    return applyLockedSubscription(
      'Unavailable',
      'Internet সংযোগ নেই এবং এই SITE_ID-এর কোনো বৈধ offline subscription cache পাওয়া যায়নি।'
    );
  }
  ...
  try{
    ...
    // Prevent stale intermediary/proxy responses.
    endpoint.searchParams.set('_ts',Date.now().toString());

    // Do not let a stalled/blocked API request keep the app forever on
    // "Subscription status যাচাই করা হচ্ছে…". After the timeout, the
    // catch block will use the last valid per-SITE_ID offline cache.
    ...
    /*
     * A successful server response is authoritative.
     * If the server says paused/expired/inactive, do NOT fall back to an
     * older cache, because that would allow an intentionally revoked license.
     */
    if(status==='active' && isExpiryValid(expiry)){
      writeSubscriptionCache(siteId,{
        status,
        expiry_date:expiry
      });

      return applyActiveSubscription(expiry,'Active');
    }

    clearSubscriptionCache(siteId);
    ...
    console.warn('Subscription online check failed; trying offline cache:',error);

    /*
     * Network/API failure is different from a successful "paused" or
     * "expired" response. Only in this failure path do we use the cached
    ...
    const timeoutOrNetwork = error?.name==='AbortError'
      ? 'Subscription server-এর উত্তর পেতে বেশি সময় লাগছে।'
      : 'Subscription server-এ পৌঁছানো যাচ্ছে না।';

    return applyLockedSubscription(
      'Unavailable',
      timeoutOrNetwork+' কোনো বৈধ offline cache নেই। Internet সংযোগ পরীক্ষা করুন।'
    );
  }
}
...
async function bootApp(){
  /*
   * Resolve/persist SITE_ID before any tenant-specific data is used.
   * This is especially important for an installed PWA, where the launch
   * URL may no longer contain ?site_id=....
   */
  const resolvedSiteId=resolveSiteId();

  if(!resolvedSiteId){
    showMissingSiteIdSetup();
    return;
  }

  // Keep the global value synchronized for all existing app functions.
  SITE_ID=resolvedSiteId;
  persistSiteId(SITE_ID);

  configureSupportLinks();
  updateDriveConnectionStatus(driveConnected);

  // checkSubscriptionStatus handles online verification and offline cache.
  const allowed=await checkSubscriptionStatus();
  if(!allowed)return;

  /*
   * Only after tenant identity and subscription access are established do
   * we hydrate/render the local tenant data.
   */
  await hydrate();
  go('home');
  ...
  if(onlineNow() && backupDirty && driveConnected){
    scheduleAutoBackup();
  }
}
