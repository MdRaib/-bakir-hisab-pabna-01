...
async function driveBackup(silent=false,retryAuth=true){
  ...
  try{
    ...
    console.error(e);
    if(e?.message==='AUTH' && retryAuth && onlineNow()){
      accessToken=null;
      driveConnected=false;
      localStorage.setItem('shudhu-baki-drive-connected','0');
      try{
        if(await ensureDriveAccess()){ driveBusy=false; return await driveBackup(silent,false); }
      }catch(re){console.error(re)}
    }
    updateBackupStatus('Backup ব্যর্থ হয়েছে। আবার চেষ্টা করুন।');
    if(!silent)alert('Backup করা যায়নি। Internet ও Google Drive অনুমতি পরীক্ষা করুন।');
    return false;
  }finally{driveBusy=false}
}
async function autoBackup(){if(subscriptionValid&&onlineNow()&&backupDirty)await driveBackup(true)}window.addEventListener('online',()=>scheduleAutoBackup());document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible')scheduleAutoBackup()});
