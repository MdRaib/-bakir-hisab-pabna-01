...
function persistSiteId(id){
  ...
  try{document.cookie=SITE_ID_COOKIE_KEY+'='+encodeURIComponent(id)+'; path=/; max-age=31536000; SameSite=Lax'}catch(e){}
}
function getSiteIdFromAnyUrl(){
  const params=new URLSearchParams(window.location.search);
  const direct=String(params.get('site_id')||params.get('siteId')||params.get('site')||'').trim();
  if(direct)return direct;
  try{
    const hash=new URLSearchParams(String(window.location.hash||'').replace(/^#/,'?'));
    const h=String(hash.get('site_id')||hash.get('siteId')||hash.get('site')||'').trim();
    if(h)return h;
  }catch(e){}
  return getSiteIdFromUrl();
}
const urlSiteId = getSiteIdFromAnyUrl();
let savedSiteId='';
try{savedSiteId=String(localStorage.getItem(SITE_ID_STORAGE_KEY)||'').trim()}catch(e){}
if(!savedSiteId){try{savedSiteId=String(sessionStorage.getItem(SITE_ID_STORAGE_KEY)||'').trim()}catch(e){}}
if(!savedSiteId)savedSiteId=readSiteIdCookie();
const configSiteId = String((typeof APP_CONFIG !== 'undefined' && APP_CONFIG && APP_CONFIG.SITE_ID) || '').trim();
SITE_ID = urlSiteId || savedSiteId || configSiteId;
if (SITE_ID) persistSiteId(SITE_ID);

if (!SITE_ID) {
  // Installed PWAs can lose the query string from their launch URL.
  // Ask once for the client SITE_ID instead of showing a permanent Access Denied.
  setTimeout(() => {
    document.body.innerHTML = `
      <div style="position:fixed;inset:0;display:flex;align-items:center;justify-content:center;background:#f6f7fb;z-index:999999;font-family:Arial,sans-serif;padding:20px;box-sizing:border-box;">
        <div style="width:min(420px,100%);background:#fff;border-radius:18px;padding:28px;box-shadow:0 10px 35px rgba(0,0,0,.10);text-align:center;">
          ...
          <p style="color:#555;line-height:1.6;margin:0 0 18px">এই ইনস্টল করা অ্যাপের জন্য SITE_ID পাওয়া যায়নি। আপনার ক্লায়েন্ট লিংকের <b>site_id</b> অংশটি নিচে লিখুন।</p>
          <input id="pwaSiteIdInput" type="text" autocomplete="off" placeholder="যেমন: client_pabna_01" style="width:100%;box-sizing:border-box;padding:13px 14px;border:1px solid #ddd;border-radius:10px;font-size:16px;margin-bottom:12px">
          <button id="pwaSiteIdSave" style="width:100%;padding:13px;border:0;border-radius:10px;background:#111;color:#fff;font-size:16px;font-weight:700;cursor:pointer">অ্যাপ চালু করুন</button>
          <p id="pwaSiteIdError" style="display:none;color:#c62828;margin:12px 0 0">SITE_ID লিখুন।</p>
        </div>
      </div>`;
    ...
    const submit=()=>{
      ...
      if(!id){document.getElementById('pwaSiteIdError').style.display='block';return;}
      persistSiteId(id);
