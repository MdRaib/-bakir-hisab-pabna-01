// ইউআরএল থেকে site_id ধরে স্টোরেজ আলাদা করা হলো
const urlParams = new URLSearchParams(window.location.search);
const siteId = urlParams.get('site_id') || 'default_user';

const KEY = "shudhu-baki-hisab-v2_" + siteId, IDB_NAME = "shudhu-baki-hisab-offline-v2_" + siteId, IDB_STORE = "app";

let db = JSON.parse(localStorage.getItem(KEY) || "null") || { customers: [], settings: {} };
db.customers ??= [];
db.settings ??= {};
...
function go(page) { selectedPage = page; $$('.page').forEach(x => x.classList.add('hidden'));$('#page-' + page)?.classList.remove('hidden'); $$('.nav button').forEach(x => x.classList.toggle('active', x.dataset.page === page)); if (page === 'customers') renderCustomers(); if (page === 'memo') initMemo(); if (page === 'payment') resetPayment(); if (page === 'hal') resetHal(); if (page === 'hal-all') renderHalAll(); if (page === 'history') renderHistory(historyCustomerId); window.scrollTo({ top: 0, behavior: 'smooth' }) }$$('[data-page]').forEach(b => b.addEventListener('click', () => go(b.dataset.page)));
function render() { $('#shopName').textContent = SHOP_CONFIG.name; $('#totalDue').textContent = money(totalDue()); $('#dueCustomerCount').textContent = db.customers.filter(c => dueOf(c) > 0).length.toLocaleString('bn-BD'); renderHomeResults(); if (selectedPage === 'customers') renderCustomers() }
...
$('#paySearch').addEventListener('input', () => pickerResults('#paySearch', '#payResults', selectPayCustomer)); $('#payDate').addEventListener('change', updatePayDateDisplay); $('#savePayment').onclick = () => { let c = findCustomer($('#payCustomerId').value), d = Math.max(0, dueOf(c \vert{}\vert{} {})), a = Number($('#payAmount').value || 0), pd = $('#payDate').value \vert{}\vert{} today; if (!c) return alert('কাস্টমার নির্বাচন করুন।'); if (d <= 0) return alert('এই কাস্টমারের কোনো বাকি নেই।'); if (!a \vert{}\vert{} a <= 0) return alert('জমার টাকা দিন।'); if (a > d) return alert('জমা টাকা বর্তমান বাকি থেকে বেশি হতে পারবে না।'); c.ledger.push({ id: Date.now(), date: pd, type: 'paid', amount: a, note: $('#payNote').value.trim() || 'বাকি পরিশোধ' }); save(); render(); renderHalStatusLists(); toast('টাকা জমা হয়েছে এবং বাকি কমেছে।'); resetPayment() };
function renderHalStatusLists() {
  ...
  $('#halUnpaidList').innerHTML = preview(unpaid);   $$('[data-hal-pick]').forEach(b => b.onclick = () => openHistory(b.dataset.halPick));$('#halPaidAllBtn').onclick = () => { window.halAllMode = 'paid'; go('hal-all') };
...
function halCardMarkup(c, pdfMode = false) {
  ...
  const shop = SHOP_CONFIG.name || 'আমাদের প্রতিষ্ঠান';
  const owner = SHOP_CONFIG.owner || '';
  const phone = SHOP_CONFIG.phone || '';
  const address = SHOP_CONFIG.address || '';
