...
function currentShopProfile(){
  ...
  const site=String(window.SITE_ID||SITE_ID||'default').trim()||'default';
  db.settings??={};
  db.settings.shopProfiles??={};
  const saved=db.settings.shopProfiles[site]||{};
  return {name:String(saved.name??base.name),owner:String(saved.owner??base.owner),phone:String(saved.phone??base.phone),address:String(saved.address??base.address)};
}
function saveShopProfile(profile){
  const site=String(window.SITE_ID||SITE_ID||'default').trim()||'default';
  db.settings??={};db.settings.shopProfiles??={};
  db.settings.shopProfiles[site]={name:String(profile.name||'').trim(),owner:String(profile.owner||'').trim(),phone:String(profile.phone||'').trim(),address:String(profile.address||'').trim()};
}
